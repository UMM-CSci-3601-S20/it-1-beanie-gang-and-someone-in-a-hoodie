...
export class AddNoteComponent implements OnInit {
  ...
  submitForm() {
    let noteToAdd: Note = this.addNoteForm.value;
    noteToAdd.ownerID = 0 //get owner ID from somewhere, put here
    noteToAdd.addDate = new Date();

    this.noteService.addNewNote(this.addNoteForm.value).subscribe(newID => {
      this.snackBar.open('Added Note ' + this.addNoteForm.value.status, null, {
        duration: 2000,
      });
